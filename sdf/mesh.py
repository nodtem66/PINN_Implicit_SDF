...
def generate(*args, **kwargs):
    ...
    if method == 1:
        return _pyscaffolder_generate(*args, **kwargs)
    return _skimage_generate(*args, **kwargs)

def _pyscaffolder_generate(f, step=0.01, bounds=None, verbose=True, clean=True):
    import PyScaffolder
    ...
    try:
        ...
        if verbose:
            ...
            print('\n%d triangles in %g seconds' % (len(f), seconds))
...
def show_slice(*args, **kwargs):
    ...
    im = plt.imshow(a, extent=extent, origin='lower', cmap='coolwarm')
    plt.xlabel(axes[0])
    plt.ylabel(axes[1])
    plt.colorbar(im)
    plt.show()
